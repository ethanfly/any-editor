...
| 功能               | 说明                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------ |
| Markdown WYSIWYG | 基于 [HorseMD](https://github.com/BND-1/horseMD) 的 Milkdown Crepe 引擎：斜杠命令、表格、公式、Mermaid、源码保真 |
...
| 编辑器      | Monaco Editor                                                      |
| Markdown | Milkdown Crepe（WYSIWYG，对齐 HorseMD）+ marked（预览/导出）+ KaTeX + Mermaid |
| PDF      | pdfjs-dist                                                         |
| 桌面框架     | Tauri 2 (Rust)                                                     |
| 包管理      | pnpm                                                               |
...
| `build.yml`        | workflow\_call / workflow\_dispatch / tag | 三平台编译 + 发布 Release  |
...
│   │   ├── WysiwygEditor.tsx    # Markdown WYSIWYG（适配层）
...
│   │   ├── Outline.tsx          # 大纲
│   │   └── ...
│   ├── md-editor/               # HorseMD Milkdown Crepe 编辑栈（移植）
│   ├── hooks/                   # 自定义 Hooks
│   ├── types/                   # 类型与设置
│   └── utils/                   # 格式化、导出、工作区等工具
